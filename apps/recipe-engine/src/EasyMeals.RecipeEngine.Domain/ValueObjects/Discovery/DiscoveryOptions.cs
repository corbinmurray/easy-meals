...
public sealed record DiscoveryOptions
{
    ...
    /// </summary>
    public DiscoveryOptions WithMaxDepth(int maxDepth)
    {
        return this with { MaxDepth = ValidateMaxDepth(maxDepth) };
    }

    /// <summary>
    ///     Creates a copy with updated max URLs
    /// </summary>
    public DiscoveryOptions WithMaxUrls(int maxUrls)
    {
        return this with { MaxUrls = ValidateMaxUrls(maxUrls) };
    }
    ...
    public static DiscoveryOptions Conservative => new(
        ...
        delayBetweenRequests: TimeSpan.FromMilliseconds(2000));
