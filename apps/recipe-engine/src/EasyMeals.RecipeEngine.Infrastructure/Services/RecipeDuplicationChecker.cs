...
public class RecipeDuplicationChecker : IRecipeDuplicationChecker
{
    ...
    {
        return await _fingerprintRepository.ExistsAsync(url, providerId, cancellationToken);
    }
    ...
    public async Task<RecipeFingerprint?> GetExistingFingerprintAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        return await _fingerprintRepository.GetByUrlAsync(url, providerId, cancellationToken);
    }
}
