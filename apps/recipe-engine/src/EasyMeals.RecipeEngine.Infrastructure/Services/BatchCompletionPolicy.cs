...
public class BatchCompletionPolicy : IBatchCompletionPolicy
{
    public bool ShouldCompleteBatch(RecipeBatch batch, DateTime currentTime)
    {
        ...
        var reason = GetCompletionReason(batch, currentTime);
    ...
    public BatchCompletionReason GetCompletionReason(RecipeBatch batch, DateTime currentTime)
    {
        ...
        if (sizeReached && timeExceeded)
        {
            return BatchCompletionReason.Both;
        }
        if (sizeReached)
        {
            return BatchCompletionReason.BatchSizeReached;
        }
        if (timeExceeded)
        {
            return BatchCompletionReason.TimeWindowExceeded;
        }
