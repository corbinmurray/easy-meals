...
public class IngredientNormalizationService : IIngredientNormalizer
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var cacheKey = GetCacheKey(providerId, providerCode);

        // Check cache first
        if (TryGetFromCache(cacheKey, out var cachedValue))
        {
            _logger.LogDebug(
        ...
        // Query database
        var mapping = await _repository.GetByCodeAsync(providerId, providerCode, cancellationToken);
        ...
        var canonicalForm = mapping.CanonicalForm;
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var uniqueCodes = providerCodes.Distinct().ToList();
        ...
        {
            var canonicalForm = await NormalizeAsync(providerId, code, cancellationToken);
            result[code] = canonicalForm;
        }

        var mappedCount = result.Values.Count(v => v is not null);
        var unmappedCount = result.Values.Count(v => v is null);
    ...
    private void AddToCache(string cacheKey, string? canonicalForm)
    {
        lock (_cacheLock)
        {
            ...
            {
                var oldestKey = _cacheOrder.Last?.Value;
