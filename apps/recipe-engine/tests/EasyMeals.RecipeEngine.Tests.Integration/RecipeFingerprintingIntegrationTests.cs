...
public class RecipeFingerprintingIntegrationTests : IAsyncLifetime
{
	...
	public async Task InitializeAsync()
	{
		...
		var collection = _database.GetCollection<RecipeFingerprintDocument>("fingerprints");
	...
	public async Task GenerateAndStoreFingerprint_PersistsToMongoDB()
	{
		...
		var savedFingerprint = await _repository!.GetByUrlAsync(url, providerId);
	...
	public async Task IsDuplicate_ExistingFingerprint_ReturnsTrue()
	{
		...
		var isDuplicate = await _fingerprintService.IsDuplicateAsync(fingerprint);
	...
	public async Task SameContent_GeneratesSameFingerprintAndDetectsDuplicate()
	{
		...
		var isDuplicate = await _fingerprintService.IsDuplicateAsync(fingerprint2);
	...
	public async Task MultipleProviders_DifferentFingerprintsForSameUrl()
	{
		...
		var saved1 = await _repository!.GetByUrlAsync(url, provider1);
		var saved2 = await _repository.GetByUrlAsync(url, provider2);
	...
	public async Task CountByProvider_ReturnsCorrectCount()
	{
		...
		for (int i = 0; i < 3; i++)
		{
			...
			var fingerprint = _fingerprintService!.GenerateFingerprint(url, $"Recipe {i}", $"Description {i}");
		...
		// Act
		var count1 = await _repository!.CountByProviderAsync(provider1);
		var count2 = await _repository.CountByProviderAsync(provider2);
