...
public class ProviderConfigurationCachingTests : IAsyncLifetime
{
	...
	{
		// Arrange
		var config1 = CreateProviderDocument("provider_001", true);
		var config2 = CreateProviderDocument("provider_002", true);
	...
	{
		// Arrange
		var enabledConfig = CreateProviderDocument("provider_enabled", true);
		var disabledConfig = CreateProviderDocument("provider_disabled", false);
	...
	{
		// Arrange
		var config = CreateProviderDocument("provider_specific", true);
		await _repository!.InsertOneAsync(config);

		// Act
		var result = await _loader!.GetByProviderIdAsync("provider_specific");
	...
	{
		// Arrange
		var config = CreateProviderDocument("provider_001", true);
		await _repository!.InsertOneAsync(config);
	...
	{
		// Arrange
		var config = CreateProviderDocument("provider_concurrent", true);
		await _repository!.InsertOneAsync(config);
