...
public class RecipeFingerprintServiceTests
{
	...
	public void GenerateFingerprint_ValidInputs_ReturnsSha256Hash()
	{
		...
		// Act
		var fingerprint = _fingerprintService.GenerateFingerprint(url, title, description);
	...
	public void GenerateFingerprint_UppercaseUrl_NormalizesToLowercase()
	{
		...
		// Act
		var fingerprint1 = _fingerprintService.GenerateFingerprint(urlUppercase, title, description);
		var fingerprint2 = _fingerprintService.GenerateFingerprint(urlLowercase, title, description);
	...
	public void GenerateFingerprint_UrlWithQueryParams_RemovesQueryParams()
	{
		...
		// Act
		var fingerprint1 = _fingerprintService.GenerateFingerprint(urlWithParams, title, description);
		var fingerprint2 = _fingerprintService.GenerateFingerprint(urlWithoutParams, title, description);
	...
	public void GenerateFingerprint_TitleWithWhitespace_TrimsAndLowercases()
	{
		...
		// Act
		var fingerprint1 = _fingerprintService.GenerateFingerprint(url, titleWithWhitespace, description);
		var fingerprint2 = _fingerprintService.GenerateFingerprint(url, titleNormalized, description);
	...
	public void GenerateFingerprint_LongDescription_TakesFirst200Chars()
	{
		...
		// Act
		var fingerprintShort = _fingerprintService.GenerateFingerprint(url, title, shortDescription);
		var fingerprintLong = _fingerprintService.GenerateFingerprint(url, title, longDescription);

		// Verify fingerprint uses truncated description
		var expectedFingerprint = _fingerprintService.GenerateFingerprint(url, title, truncatedDescription);
	...
	#region T118: Unit test for fingerprint duplicate detection (same content → same hash)
	...
	public void GenerateFingerprint_SameContent_ProducesSameHash()
	{
		...
		var fingerprint1 = _fingerprintService.GenerateFingerprint(url, title, description);
		var fingerprint2 = _fingerprintService.GenerateFingerprint(url, title, description);
	...
	public void GenerateFingerprint_DifferentUrls_ProducesDifferentHashes()
	{
		...
		// Act
		var fingerprint1 = _fingerprintService.GenerateFingerprint(url1, title, description);
		var fingerprint2 = _fingerprintService.GenerateFingerprint(url2, title, description);
	...
	public void GenerateFingerprint_DifferentTitles_ProducesDifferentHashes()
	{
		...
		// Act
		var fingerprint1 = _fingerprintService.GenerateFingerprint(url, title1, description);
		var fingerprint2 = _fingerprintService.GenerateFingerprint(url, title2, description);
	...
	public void GenerateFingerprint_DifferentDescriptions_ProducesDifferentHashes()
	{
		...
		// Act
		var fingerprint1 = _fingerprintService.GenerateFingerprint(url, title, description1);
		var fingerprint2 = _fingerprintService.GenerateFingerprint(url, title, description2);
