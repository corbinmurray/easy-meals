...
public class EasyMealsRepositoryBuilder
{
    ...
    internal EasyMealsRepositoryBuilder(IServiceCollection services)
    {
    ...
    /// <summary>
    ///     Adds default indexes for all registered repositories
    ///     Uses EasyMeals standard indexing strategy for optimal performance
    /// </summary>
    /// <returns>Builder for method chaining</returns>
        ...
        where TDocument : BaseDocument
    {
        ...
        {
            var database = serviceProvider.GetRequiredService<IMongoDatabase>();
            var collection = database.GetCollection<TDocument>(GetCollectionName<TDocument>());
    ...
    public EasyMealsIndexBuilder ConfigureIndexes()
    {
        return new EasyMealsIndexBuilder(this);
    }
    ...
    public async Task<IServiceCollection> EnsureDatabaseAsync()
    {
        ...
        try
        {
            ...
            // Create indexes
            foreach (var indexCreator in _indexCreators)
            {
    ...
    private void RegisterRepositories()
    {
        ...
        foreach (var (documentType, permissions) in _repositories)
        {
            RegisterRepositoryForDocument(documentType, permissions);
        }

        // Register shared repositories
        foreach (var sharedRepoType in _sharedRepositories)
        {
    ...
    private void RegisterRepositoryForDocument(Type documentType, RepositoryPermissions permissions)
    {
        ...
        var mongoRepoType = typeof(IMongoRepository<>).MakeGenericType(documentType);
        var readOnlyRepoType = typeof(IReadOnlyMongoRepository<>).MakeGenericType(documentType);
        var implType = typeof(MongoRepository<>).MakeGenericType(documentType);
    ...
    private void RegisterSharedRepositoryType(Type sharedRepoType)
    {
        ...
        if (sharedRepoType.Name == nameof(IRecipeRepository))
        {
            _services.AddScoped<IRecipeRepository, RecipeRepository>();
        }
    ...
    {
        if (_repositories.Count > 0 || _sharedRepositories.Count > 0)
        {
...
public class EasyMealsIndexBuilder
{
    ...
    public EasyMealsRepositoryBuilder BuildIndexes()
    {
        return _repositoryBuilder;
    }
}
