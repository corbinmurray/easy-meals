...
### User Story 3 - Complete & Finalize Recipe Processing Saga (Priority: P1)

**Context**: The recipe processing saga orchestrates a multi-step workflow (discovery → fingerprinting → processing → persistence) but is currently incomplete. It must coordinate these steps, handle failures gracefully, maintain state across restarts, and provide visibility into processing progress.

**Why this priority**: The saga is the core orchestrator for the entire recipe engine. Without completion, the engine cannot reliably process recipes, recover from failures, or provide operational visibility. This is a blocker for production deployment.

**Independent Test**: Can be fully tested by simulating the full processing workflow (discovery → fingerprinting → processing → persistence) with injected failures at each stage, verifying that the saga transitions correctly through states, applies compensating transactions on failure, logs progress, and can resume from a saved state after restart.

**Acceptance Scenarios**:

1. **Given** the saga starts processing, **When** all steps succeed, **Then** the saga transitions through all states (Discovering → Fingerprinting → Processing → Persisting → Completed) and persists the final state
2. **Given** the saga is processing URLs and encounters a network error during fingerprinting, **When** the error is retried and succeeds, **Then** the saga continues from the failed step without reprocessing previous steps
3. **Given** the saga encounters a permanent failure (e.g., invalid recipe data), **When** the failure is non-recoverable, **Then** the saga logs the error with context, records the failed item for manual review, and continues processing remaining items
4. **Given** the saga is processing 100 recipes and the application restarts after 45 recipes, **When** the saga resumes, **Then** it loads the previous state and continues from recipe 46 (no data loss or reprocessing)
5. **Given** processing completes for a batch, **When** the saga finishes, **Then** it publishes a domain event indicating completion with metadata (recipes processed, success rate, processing time)
...
- **Provider Configuration**: Settings for a specific recipe provider including batch size, time window, delays, timeouts, retry counts, enabled status, and discovery strategy (static crawl, dynamic crawl, or API)
...
- **Discovery Strategy**: Configurable per provider; defines how recipe URLs are found (static site crawling, dynamic JavaScript-rendered site crawling with recursive link discovery, or API-based retrieval); includes recipe-root URL configuration for crawling strategies
